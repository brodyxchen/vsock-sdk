...
func TestKClientMRoutineNReq(t *testing.T) {
	LaunchCustomExampleServer(7070, time.Second, time.Second, time.Second*10, true, func(bytes []byte) []byte {
		msg := string(bytes)
	...
	for k := 0; k < K; k++ {
		go func(kk int) {
			cli := NewClient(time.Millisecond * 100)
			...
			for m := 0; m < M; m++ {
				go func(kkk, mmm int) {
					for j := 0; j < N; j++ {
						...
						connIndex, rspBytes, err := cli.SendTest(addr, 1, []byte(req), deadline)
